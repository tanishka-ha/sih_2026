...
try:
    from .models import BoundingBox, ELARegion
except ImportError:
    from models import BoundingBox, ELARegion
...
def compute_ela_map(
    ...
    # Re-compress via Pillow in memory to match JPEG encoding behavior accurately without disk file locks
    buffer = io.BytesIO()
    ...
    recompressed = cv2.imdecode(file_bytes, cv2.IMREAD_COLOR)

    if recompressed is None:
        raise RuntimeError("Failed to decode re-compressed image from memory")
